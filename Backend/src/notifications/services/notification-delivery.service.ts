...
import { NotificationChannel, NotificationStatus, NotificationPriority } from '../types/notification.types';
...
export class NotificationDeliveryService {
  ...
  async deliverNotification(notification: Notification): Promise<boolean> {
    ...
    if (!preferences.globalEnabled) {
      this.logger.log(`Notification delivery skipped for user ${userId} - global disabled`);
      return false;
    }

    // Check if user is muted
    if (preferences.mutedUntil && new Date() < new Date(preferences.mutedUntil)) {
      this.logger.log(`Notification delivery skipped for user ${userId} - user muted until ${preferences.mutedUntil}`);
      return false;
    }
    ...
    if (!typePreferences?.enabled) {
      this.logger.log(`Notification delivery skipped for user ${userId} - ${notification.type} type disabled`);
      return false;
    }
    ...
    if (notificationPriorityValue < thresholdValue) {
      this.logger.log(`Notification delivery skipped for user ${userId} - priority threshold not met`);
      return false;
    }
    ...
    for (const channel of eligibleChannels) {
      try {
        ...
        this.logger.error(`Failed to deliver notification via ${channel}: ${error.message}`);
        success = false;
      }
    }
  ...
  private async deliverInApp(notification: Notification): Promise<void> {
    try {
      ...
      this.websocketGateway.server.to(`user_${notification.userId}`).emit('notification:new', {
        id: notification.id,
  ...
  private async deliverPush(notification: Notification): Promise<void> {
    ...
    this.logger.log(`Push notification delivery attempted for user ${notification.userId}`);
  ...
  private async deliverEmail(notification: Notification): Promise<void> {
    ...
    this.logger.log(`Email notification delivery attempted for user ${notification.userId}`);
    ...
    if (!user?.email) {
      this.logger.warn(`Cannot send email notification - no email address for user ${notification.userId}`);
      await this.notificationRepository.update(notification.id, {
        status: NotificationStatus.FAILED,
      });
      return;
    ...
    try {
      ...
      this.logger.log(`Email notification delivered to user ${notification.userId} at ${user.email}`);
    } catch (error) {
      this.logger.error(`Failed to deliver email notification: ${error.message}`);
      await this.notificationRepository.update(notification.id, {
        status: NotificationStatus.FAILED,
      });
      throw error;
  ...
  private async deliverSMS(notification: Notification): Promise<void> {
    ...
    this.logger.log(`SMS notification delivery attempted for user ${notification.userId}`);
  ...
  async trackDelivery(notificationId: string, status: NotificationStatus, deliveryDetails?: any): Promise<void> {
